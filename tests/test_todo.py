...
async def test_list_todos_filter_title_should_return_5_todos(
    ...
    expected_todos = 5
    session.add_all(TodoFactory.create_batch(5, user_id=user.id, title='Test'))
    await session.commit()
...
@pytest.mark.asyncio
async def test_delete_todo(session, client, token, user):
...
@pytest.mark.asyncio
async def test_delete_error(client, token):
    response = client.delete(
...
async def test_delete_other_user_todo(session, client, token, other_user):
    todo_other_user = TodoFactory(user_id=other_user)
